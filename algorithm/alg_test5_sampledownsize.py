...
MIN = 1000  # testing parameter min
MAX = 3000  # testing parameter max
INC = 50
...
for x in range(MIN, MAX, INC):
    ...
    mem_p_d = []
    mem_p_disc = []
    sample_downsize = x  # set value to testing parameter
    times_p += 1
    times = 0
...
if PLOT_T:
    plt.figure(figsize=(16, 9))
    plt.xticks(range(length), range(MIN, MAX, INC))
    plt.plot(mic1)
    plt.plot(mic2)
    plt.plot(mic3)
    plt.plot(mic4)
