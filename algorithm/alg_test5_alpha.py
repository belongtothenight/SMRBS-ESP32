...
MIN = 0.9 # testing parameter min
MAX = 0.99 # testing parameter max
INC = 0.01
...
for x in testing_range:
    ...
    mem_p_d = []
    mem_p_disc = []
    alpha = x # set value to testing parameter
    times_p += 1
    times = 0
    while times < TOTAL_PIC:
        times += 1
        print('{0}/{1}'.format(times + (times_p-1) * TOTAL_PIC, testing_length * TOTAL_PIC))
        frames_a = []
        frames_b = []
        frames_c = []
        frames_d = []
...
if PLOT_T:
    plt.figure(figsize=(16, 9))
    plt.xticks(range(testing_length), testing_range)
    plt.plot(mic1)
    plt.plot(mic2)
    plt.plot(mic3)
    plt.plot(mic4)
